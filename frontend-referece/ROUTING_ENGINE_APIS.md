...
## Horizon vs SDP (Stellar Disbursement Platform)

- **Horizon** = Stellar’s **public REST API** for the network (accounts, ledgers, **paths**, orderbook, etc.).  
  **Use this for the routing engine** (pathfinding, liquidity, execution).
- **SDP** (Stellar Disbursement Platform) = Separate product for **disbursements, receivers, API keys, wallets**.  
  The “Create API Key” docs you saw are for **SDP**, not for Horizon. You do **not** need SDP or SDP API keys to implement the routing engine.

**Routing engine → use Horizon only.** No SDP sign-up or API keys required for pathfinding.

---

## Do You Need AI for the Routing Engine?

**No.** Core routing does **not** require AI.

- **Pathfinding** = Horizon `/paths/strict-send` and `/paths/strict-receive` return real paths and amounts. No AI.
- **Scores (cost, time, risk, liquidity)** = Deterministic logic (e.g. in `stellar.ts` / `calculateRouteScore`) using path length, orderbook, corridor risk. No AI.
- **Execution** = Build and submit a Stellar path payment (e.g. Path Payment Strict Send) via Horizon/SDK. No AI.

**Optional uses of AI (nice-to-have, not required):**

- Natural language: “Explain this route” or “Why is this route cheaper?”
- UX: Short “recommended route” explanation.
- Compliance/risk: Plain-language summary of corridor or anchor risk.

So: **implement routing with Horizon first; add AI only if you want those extra UX/compliance features.**
